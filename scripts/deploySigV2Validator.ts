...
async function main() {
  ...
  const groth16VerifierContractWrapperName = 'Groth16VerifierSigWrapper';
  const validatorContractName = 'CredentialAtomicQuerySigV2Validator';
  const Groth16VerifierSigWrapper = await ethers.getContractFactory(
    groth16VerifierContractWrapperName
  );
  const verifierWrapper = await Groth16VerifierSigWrapper.deploy();
